...
var (
	...
	//_byteUnmarshalerType = reflect.TypeOf(new(ByteUnmarshaler)).Elem()
)
...
//			vale.Set(reflect.New(fieldTypeI.Type.Elem()))
//			vale = vale.Elem()
//		}
//		err := convert(mapValue, vale, wrapErr)
//		if nil != err {
//			return err
//		}
//	}
...
//}
//
//func convert(mapValue interface{}, vale reflect.Value, wrapErr convertErrWrapper) error {
//	//vit: ValueI Type
//	vit := vale.Type()
//	//mvt: MapValue Type
...
//		vale.Set(reflect.ValueOf(mapValue))
//		return nil
//	}
//	//time.Time to string
//	switch assertT := mapValue.(type) {
//	case time.Time:
//		return handleConvertTime(assertT, mvt, vit, &vale, wrapErr)
//	}
//
//	if scanner, ok := vale.Addr().Interface().(sql.Scanner); ok {
...
//			vale.SetInt(mapValue.(int64))
//		} else if isUintSeriesType(vit.Kind()) {
//			vale.SetUint(uint64(mapValue.(int64)))
...
//			vale.SetString(strconv.FormatInt(mapValue.(int64), 10))
//		} else {
//			return wrapErr(mvt, vit)
//		}
//	case reflect.Float32:
//		if isFloatSeriesType(vit.Kind()) {
//			vale.SetFloat(float64(mapValue.(float32)))
//		} else {
//			return wrapErr(mvt, vit)
//		}
//	case reflect.Float64:
//		if isFloatSeriesType(vit.Kind()) {
//			vale.SetFloat(mapValue.(float64))
//		} else {
//			return wrapErr(mvt, vit)
//		}
//	case reflect.Slice:
//		return handleConvertSlice(mapValue, mvt, vit, &vale, wrapErr)
//	default:
//		return wrapErr(mvt, vit)
//	}
//	return nil
...
//}
//
//// byteUnmarshal Here is the type of ByteUnmarshaler
...
func before() error {
	...
	_, err = dbClient.Exec("CREATE TABLE `test` ( id INT, v TEXT )")
	if err != nil {
		fmt.Printf("error creating test schema: %v\n", err)
		os.Exit(-2)
	}

	_, err = dbClient.Exec("insert into `test` ( id, v ) values (1, \"222222\")")
	if err != nil {
		fmt.Printf("error creating test schema: %v\n", err)
		os.Exit(-2)
	}
...
func test2(entity interface{}) error {
	...
	rows, err := dbClient.Query("SELECT id, v from test limit 1")
	if err != nil {
		fmt.Printf("error: %v\n", err)
		return err
	}
...
func main() {
	...
	//err := GetOne(dbClient, &test, "test", nil)
	err := test2(&test)
	if err != nil {
		fmt.Printf("error: %v\n", err)
	}
